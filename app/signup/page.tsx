...
export default function SignUpPage() {
  ...
  const [paidCheckoutInfo, setPaidCheckoutInfo] = useState<PaidCheckoutInfo | null>(null);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (cancelled) return;
        if (!res.ok) {
          setSubmitError(json.error || 'Could not verify payment. Start checkout again or contact support.');
          window.history.replaceState({}, '', '/signup');
          return;
        }
  ...
  const validateAccountStep = (): string | null => {
    ...
    if (paidCheckoutInfo && email.toLowerCase() !== paidCheckoutInfo.email.toLowerCase()) {
  ...
  const submitFinal = async () => {
    ...
    const isPaid = formData.signupPlan === 'monthly' || formData.signupPlan === 'annual';
    ...
    try {
      ...
      if (isPaid && session?.access_token) {
        ...
        if (!linkRes.ok) {
          const lj = (await linkRes.json().catch(() => ({}))) as { error?: string };
          setSubmitError(
            lj.error ||
        ...
        <div className={`${layoutStyles.signupCard} ${layoutStyles.signupCardWide}`}>
          <div className={layoutStyles.signupWideBody}>
            ...
            <Flex direction="column" gap="4" mt="4">
              <Heading size="6">Membership</Heading>
              <Text size="2" color="gray">
                Choose your membership plan to get started.
              </Text>
              ...
              {paidCheckoutInfo && (
                <Callout.Root color="green">
                  ...
                  <Callout.Text>
                    Payment received ({paidCheckoutInfo.plan} · {paidCheckoutInfo.subscriptionStatus}). Continue
              ...
              <Flex gap="2" wrap="wrap">
                <Button
                  ...
                  onClick={() => void startGuestCheckout('monthly')}
                >
                  {checkoutStartLoading && formData.signupPlan === 'monthly' ? 'Redirecting…' : 'Monthly'}
                </Button>
                <Button
                  size="3"
                  variant={formData.signupPlan === 'annual' ? 'solid' : 'outline'}
                  disabled={checkoutStartLoading}
                  onClick={() => void startGuestCheckout('annual')}
                >
                  {checkoutStartLoading && formData.signupPlan === 'annual' ? 'Redirecting…' : 'Annual (save)'}
                </Button>
              </Flex>
              <Text size="1" color="gray">
                You&apos;ll complete checkout on Stripe, then return here to finish your profile.
              </Text>

              <Button size="3" variant="ghost" onClick={handleStartFree} style={{ alignSelf: 'flex-start' }}>
                Start free
              </Button>
            </Flex>
          </div>
          ...
          <div className={layoutStyles.signupWideBody}>
            ...
            {submitSuccess && !needsEmailVerify && (
              <Callout.Root color="green">
                ...
                <Callout.Text>Welcome! Redirecting to your account…</Callout.Text>
              </Callout.Root>
            )}
              ...
              <Flex direction="column" gap="4">
                ...
                {paidCheckoutInfo && (
                  <Callout.Root color="blue">
                    ...
                    <Callout.Text>
                      Subscribed as {paidCheckoutInfo.email}. Use the same email below.
                    </Callout.Text>
                  </Callout.Root>
                )}
                    ...
                    <Tabs.Content value="modalities">
                      <Flex direction="column" gap="4">
                        <Grid columns="2" gap="3">
                          {['Gonstead', 'Diversified', 'Activator', 'TRT', 'SOT', 'Thompson', 'Webster', 'Cox'].map(
                            (modality) => (
            ...
            {step === 5 && !submitSuccess && (
              <Flex direction="column" gap="4">
                ...
                <Callout.Root color="blue">
                  ...
                  <Callout.Text>
                    Submitting sends your profile for license review. You can still edit details later from your
                    account.
                  </Callout.Text>
                </Callout.Root>
                ...
                <Flex gap="3" justify="between" mt="4">
                  ...
                  >
                    {finalSubmitLoading ? 'Working…' : 'Create account & submit for review'}
                  </Button>
                </Flex>
              </Flex>
            )}
