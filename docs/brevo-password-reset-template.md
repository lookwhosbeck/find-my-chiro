...
| Param | Notes |
|---|---|
| `params.reset_url` | One-time recovery URL. Bounces through `/auth/callback?next=/reset-password` so the user lands on the in-app form. |
...
3. Hit `/forgot-password` in the app, submit an email that exists in
